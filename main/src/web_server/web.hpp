...
namespace fs
{
    ...
    static const String timingSettingsPage =
        ...
        "<input type=range id=SnapShotTime class=slider min=10 max=90000 step=1>"
        ...
        "    document.getElementById('SnapShotTime').value = Math.floor(data.SnapShotTime / 1000);"
        "    const sliders = ['wifiTimeout', 'noMotionTimeout', 'secureModeTimeout','SnapShotTime'];"
        "    sliders.forEach(id => {"
        "      const slider = document.getElementById(id);"
        "      const display = document.getElementById(id + 'Value');"
        ...
        "    SnapShotTime: document.getElementById('SnapShotTime').value * 1000"
        "  };"
        "  try {"
